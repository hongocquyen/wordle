document.addEventListener("DOMContentLoaded", () => {
  ...
  function showMessage(message, duration = 1000) {
    ...
    // setTimeout(() => messageDisplay.removeChild(alert),duration)
  }
  ...
  function handleSubmitWord() {
    ...
    if (currentWordArr.length !== 5) {
      ...
      shakeTiles(currentWordArr);
      return;
    }
    const currentWord = currentWordArr.join("");
    if (!dictionary.includes(currentWord)) {
      showMessage("Word is not in directionary");
      shakeTiles(currentWordArr);
      return;
    }
    ...
    currentWordArr.forEach((letter, index) => {
      setTimeout(() => {
        ...
        if (letter == word[index]) {
          ...
          keyletterEl.classList.add("correct");
        }
        //LETTER in Guessword and not in correct position
        else if (word.includes(letter)) {
          letterEl.classList.add("wrong-position");
          keyletterEl.classList.add("wrong-position");
        }
        //LETTER not in Guessword
        else {
          letterEl.classList.add("wrong");
          keyletterEl.classList.add("wrong");
        }
      }, interval * index);
    });
    ...
    if (currentWord === word) {
      ...
      showMessage("Congratulation!", 3000);
      return;
    }

    if (guessedWords.length === 6) {
      // window.alert(`You lose, the word is ${word}.`)
      showMessage("You lose! The word is " + word, 3000);
      return;
    }
    guessedWords.push([]);
  }

  function shakeTiles(currentWordArr) {
    // const currentWord = wordArr.join('');
